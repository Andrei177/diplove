...
export const Sidebar: FC<IPropsSidebar> = ({ alone, setShowSidebar }) => {

    const isPlanshet = useMediaQuery({ maxWidth: "685px" });
    const isMobile = useMediaQuery({ maxWidth: "625px" });
    const { chats } = useChatsListStore();
    const {setChatId, setOtherProfileFirstName, setOtherProfileImage } = useChatStore();
    ...
    return (
        <div className={alone ? cx(s.sidebar, s.alone) : s.sidebar}>
            ...
            <div className={s.sidebar_chats}>
                ...
                {
                    chats.length !== 0
                        ? <>{chats.map(chatInfo => <ChatInfo key={chatInfo.chat_id} chatInfo={chatInfo} onClick={() => handleClickOnChatInfo(chatInfo)} />)}</>
